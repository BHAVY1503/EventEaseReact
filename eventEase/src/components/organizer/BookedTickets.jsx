...
import "@/styles/components/BookedTickets.css";
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.5em] text-[#E11D48] animate-pulse">Syncing Ticket Manifest</p>
      </div>
    );
  }
  ...
  return (
    <TooltipProvider>
      <div className="booked-tickets-container">
        <div className="booked-tickets-content">
          {/* HEADER SECTION */}
          <motion.div 
          ...
          >
            <div className="manifest-status-badge">
              ...
              <span>Real-time Manifest Stream</span>
            </div>
            ...
            <div className="manifest-title-row">
              <div className="manifest-title-box">
                ...
                <p className="manifest-subtitle">Platform-wide ticket deployment and entity oversight.</p>
              </div>

              <div className="manifest-stats-box">
                <div className="stat-group-manifest">
                  <p>Global Assets</p>
                  <p>{tickets.length}</p>
                </div>
                <Zap className="manifest-zap-icon" />
              </div>
            </div>
          </motion.div>
          ...
          {tickets.length === 0 ? (
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="empty-manifest-box"
            >
              <ShieldAlert />
              <p>Zero active ticket entities detected in registry.</p>
            </motion.div>
          ) : (
                ...
                {tickets.map((ticket, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="ticket-visual-segment">
                        <img 
                          src={event?.eventImgUrl} 
                          alt="" 
                        />
                        <div className="ticket-visual-overlay" />
                        
                        <div className="ticket-status-badge-floating">
                          <div className={cn(
                            "status-badge-ticket",
                            status === "past" ? "cancelled" : "active"
                          )}>
                            {status === "past" ? "PAST" : status.toUpperCase()}
                          </div>
                        </div>

                        <div className="ticket-title-floating">
                          <div className="ticket-type-label">
                            {event?.eventType || "SESSION"}
                          </div>
                          <h3 className="ticket-title-text">
                            {event?.eventName || "RESERVED NODE"}
                          </h3>
                        </div>
                      </div>

                      <div className="ticket-info-segment">
                        <div className="ticket-details-list">
                          <div className="ticket-detail-item">
                            <div className="detail-icon-box">
                              <User />
                            </div>
                            <p>{ticket.userId?.fullName || "AUTHORIZED GUEST"}</p>
                          </div>
                          <div className="ticket-detail-item">
                            <div className="detail-icon-box">
                              <Calendar />
                          ...
                          <div className="ticket-detail-item">
                            ...
                            <p>{event?.cityId?.name || "Global Node"}</p>
                          </div>
                        </div>

                        <div className="ticket-id-tag">
                          <p>#{ticket._id.slice(-8).toUpperCase()}</p>
                          <div className="ticket-quantity-badge">
