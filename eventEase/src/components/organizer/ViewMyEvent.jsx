...
export const ViewMyEvent = () => {
  ...
  const EventCard = ({ event }) => {
    ...
    return (
      <Card className="group cursor-pointer hover:shadow-xl hover:-translate-y-2 transition-all duration-300 bg-white/70 backdrop-blur-sm border-white/50 overflow-hidden">
...
//   // Fetch organizer ID from backend
//   const getOrganizerId = async () => {
//     try {
//       const res = await axios.get("/organizer/organizer/self", {
//         headers: { Authorization: `Bearer ${token}` },
...
//                         <Badge variant="outline">{event.eventType}</Badge>
//                         <Badge variant="secondary">{event.eventCategory}</Badge>
//                         {event.isApproved && <Badge variant="destructive">Approved</Badge>}
//                       </CardDescription>
//                     </CardHeader>
